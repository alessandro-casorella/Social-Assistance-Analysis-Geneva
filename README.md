...
At this stage, the project already highlights several important challenges : for instance, unconditional parallel trends do not hold.

To solve these issues, flexible methods are being considered. For instance, Double Machine Learning (Double Lasso for average effects, Causal Random Forest with CLAN for heterogeneity) could help address selection and composition biases. Synthetic Control methods are also considered to deal with structural differences and diverging trends.

This work is still a draft. Not only does it need to be completed, but the existing code may also be revised. In particular, I may refine and streamline it (, for instance, reduce redundancies), as well as improve certain aspects that require further development (implementing a leakage-safe NA imputation pipeline, refining the forward-looking score, or making the comments clearer and more concise-. A conceptual report will also be released.
