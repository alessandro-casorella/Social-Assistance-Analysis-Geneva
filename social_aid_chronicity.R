...
path <- "/Users/alessandro/Desktop/shplong_p_user.dta"
...
View(variable_library) # we can look here at the set of variables
##########
##########
...
#   and hhmove (moved since last interview - 1 year). 
...
path_hous <- "/Users/alessandro/Desktop/shplong_h_user.dta"
...
View(variable_library_hous) # to see the variables
...
View(variable_library_updated) # it worked ! ! ! 
saveRDS(df_merged, "/Users/alessandro/Desktop/df_merged_p_h.rds") 
# We save the file, in order to avoid redoing the heavy previous steps next time.
# We will only need to upload :
df_merged = readRDS("/Users/alessandro/Desktop/df_merged_p_h.rds")
##########
##########
...
saveRDS(df_cleaned, "/Users/alessandro/Desktop/df_cleaned.rds") 
# We save the file, in order to avoid redoing the heavy previous steps next time.
# We will only need to upload :
df_cleaned = readRDS("/Users/alessandro/Desktop/df_cleaned.rds")
...
meta_source <- read_stata("/Users/alessandro/Desktop/shplong_p_user.dta", n_max = 0)
...
meta_source_h <- read_stata("/Users/alessandro/Desktop/shplong_h_user.dta", n_max = 0)
...
##########
##########
############# The following section is too heavy to run. 
############# For this reason I will not control for time-invariant
###########   effects, in the DML proposal
# Mundlak transformations:
# The FML algorithms, unlike classic diff-in-diff, 
# cannot implicitly introduce time-invariant effects through a constant. 
# We will thus introduce them ourselves. 

# For each variable, we compute the temporal mean per individual from 1999 to 2006.

gc()
# We transform the factors into dummies, in order to average these. 
# We use a temporary object (df_temp) to keep df intact
df_temp <- df %>%
  dummy_cols(select_columns = vars_facteurs_finaux, 
             remove_selected_columns = TRUE, 
             ignore_na = FALSE) 
rm(df_final)
gc()
# We apply the transformation to the future controls : not to the treatment, neither
# to the outcomes or the weights :
vars_exclues <- c("idpers", "year", "canton", "wicss", "idispy") 
vars_to_average <- setdiff(names(df_temp), vars_exclues)

var_blocks <- split(vars_to_average, ceiling(seq_along(vars_to_average) / 100))
# We create a list to store blocks of means instead of joining them one by one to the big dataframe
mundlak_list <- list()
for (i in seq_along(var_blocks)) {
  message(paste("Computing Mundlak block", i, "of", length(var_blocks)))
  
  # We compute the means on a filtered subset to save memory
  mundlak_list[[i]] <- df_temp %>%
    filter(year <= 2006) %>% 
    group_by(idpers) %>%
    summarise(across(all_of(var_blocks[[i]]), 
                     ~mean(., na.rm = TRUE), 
                     .names = "{.col}_mean")) %>%
    ungroup()
  # Clear temporary memory within the loop
  gc()
}
# We merge all the small mean tables together first (very fast)
all_means <- Reduce(function(x, y) left_join(x, y, by = "idpers"), mundlak_list)
rm(mundlak_list)
rm(df_temp)
gc()
# We join the complete block of means to the original dataset in ONE step
# df still has its original factor columns
df <- df %>%
  left_join(all_means, by = "idpers")

rm(all_means)
gc()
##################    End of the section heavy to run. 
#################
################
##########
##########
...
# Concerning the idea of a synthetic control method,
# refer to the report.
...
saveRDS(df, "/Users/alessandro/Desktop/df.rds") 
# We save the file, in order to avoid redoing the heavy previous steps next time.
# We will only need to upload :
df = readRDS("/Users/alessandro/Desktop/df.rds")
...
# Although this number often include a same recorded across differente years,
# 5691 is an appropriate number of observations for the treatment group.

# We also have 219 treated observations concerning people who,
...
# This likely suggests a higher prevalence of the "working poor" in Geneva
...
# Forward-looking score (clean + ML-ready version)
df$score_fw <- with(df, {
...
# we will introduce in our DML models some canton-specific linear trends
...
df_baseline <- df
# We will only work on the iwely related outcome. 
...
# La version la plus "Safe" pour un économiste :
df_baseline <- df_baseline %>%
  group_by(idpers) %>%
  mutate(
...
# In the report, we discuss the issues coming from the pre-treatment 
# controls handling, and on how the DML does solve it. 
